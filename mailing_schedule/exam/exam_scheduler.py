...
if __name__ == '__main__':
    ...
    items = os.listdir(op.join(files_path, 'theory'))
    theory_files = [op.join(files_path, 'theory', f)
             for f in items if op.isfile(op.join(files_path, 'theory', f))
                    and f != 'text.txt']
    ...
    sender = Sender(
        params['your_mail'], params['your_password'], params['subject_theory'],
        theory_common_text, recipients, theory_files, params['as_attachment'],
        params.get('randomize', False))
    scheduler = Scheduler(params['theory_date_time'], sender)
    scheduler.run()
